    ...
    <div class="singleArticle">
        <div class="minicontainer">
			<span class="breadcrumbs">
                ...
                @if (!empty($article->category->ancestors))
                    @foreach($article->category->ancestors as $parent_category)
                        <a href="{{ route('analyzes.index', $parent_category->slug) }}">{{ $parent_category->title }}</a>
                    @endforeach
                @endif
                <a href="{{ route('analyzes.index', $article->category->slug) }}">{{ $article->category->title }}</a>
			</span>
            ...
            <div class="shareContainer">
                ...
                <span class="iconsBox">
							<a class="fb-share" href=""><i class="fab fa-facebook-f"></i></a>
							<a href="https://twitter.com/intent/tweet?text={{ trans('blog::front.twitter_share_msg', ['title' => $article->title, 'url' => route('blog.view', ['article_slug' => $article->slug])]) }}">
                                <i class="fab fa-twitter"></i>
                            </a>
{{--                            {{ dd(Request::url()) }}--}}
                            <a href="https://telegram.me/share/url?url={{ Request::url() }}&text={{ $article->meta_title }}" target="_blank"><i class="fab fa-telegram-plane"></i></a>
						</span>
                <span class="counter">
            ...
            <div class="descriptionContainer mt-3">
                <div class="description">
                    @if (!empty($article->mini_description))
                        <h1 class="textStrong">{{ $article->mini_description }}</h1>
                    @endif
...
{{--                            {{ dd(Request::url()) }}--}}
                    ...
                    @if (!empty($article->next))
                        <a class="anotherArticle"
                           href="{{ route('analyzes.view', ['category_slug' => $article->next->category->slug, 'article_slug' => $article->next->slug]) }}">{{ trans('blog::front.read_more') }} {{ $article->next->title }}</a>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="commentsContainer">
        <div class="minicontainer">
            ...
            <div class="formContainer">
                {!! Form::open(['url' => route('analyzes.store_comment'), 'method' => 'post', 'files'=> false, 'id' => 'post-new-comment']) !!}
                {{--{{ csrf_field() }}--}}
    ...
    @if ($random_articles->isNotEmpty())
        <div class="moreNewsContainer">
            <div class="minicontainer">
                <span class="topTitle">{{ trans('analyzes::front.more_from', ['title' => $article->category->title]) }}</span>
            </div>
            <div class="minicontainer">
                <div class="leftBlock">
                    ...
                    @foreach($random_articles as $random_article)
                        <a class="article" href="{{ route('analyzes.view', ['category_slug' => $random_article->category->slug, 'article_slug' => $random_article->slug]) }}" title="{{$random_article->title}}">
							...
							<span class="infoCol">
								<span class="icon"><i class="far fa-clock"></i></span>
								<strong>{{$random_article->estimate_reading_time(strip_tags($random_article->description))}}
                                    . {{ trans('index::front.reading') }}</strong>
							</span>
						</span>
                            </div>
                    ...
                    <div class="lastestNewsList">
                        ...
                        <ul class="latestNewsList">
                            @foreach($popular_articles as $popular_article)
                                <li>
                                    <a class="article"
                                       href="{{ route('analyzes.view', ['slug' => $popular_article->category->slug, 'article_slug' => $popular_article->slug]) }}" title="{{$popular_article->title}}">
										...
										<span class="infoBox">
											...
											<span class="infoCol">
												<span class="icon"><i class="far fa-clock"></i></span>
												<strong>{{$popular_article->estimate_reading_time(strip_tags($popular_article->description))}}
                                                    . {{ trans('index::front.reading') }}</strong>
											</span>
										</span>
									</span>
    ...
    <div class="modal fade" id="post-comment-modal">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                {!! Form::open(['url' => route('analyzes.store_comment'), 'method' => 'post', 'files'=> true, 'id' => 'post-comment-form']) !!}
